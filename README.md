...
**Book Receipt Screensaver** 是一个为 KOReader 定制的屏保插件，在设备休眠时自动生成一张「阅读收据」，清晰展示当前书籍的阅读进度、章节信息、剩余时间、今日阅读时长等关键数据，让阅读轨迹一目了然。
...
1. 将插件放入 `koreader/patches/` 目录
2. 在 KOReader 设置 → 屏保 中选择「书籍收据」
3. 支持在「书籍收据设置」中自定义背景、内容模式、封面缩放等
