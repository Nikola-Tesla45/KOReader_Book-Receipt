...
local function buildReceipt(ui, state)
    ...
    -- ====== 尺寸适度放大 ======
    local widget_width = Screen:getWidth() * 0.75   -- 75% 宽度
    ...
    local db_font_size_big = 28    -- 标题
    local db_font_size_mid = 20    -- 正文
    local db_font_size_small = 16  -- 小字
    local db_padding = 24          -- 内边距
    local db_padding_internal = 8  -- 内部间距
    -- ============================
    ...
    -- 使用 ◆ 和 ◇
    local bookbox = databox(_("◆ 书籍"), bookboxtitle, page_no_numeric, page_total_numeric, book_time_left, page_no_display, page_total_display, {
    ...
    local chapterbox = content_mode ~= CONTENT_MODE_HIGHLIGHT_PROGRESS and databox(_("◇ 章节"), chapter_title, chapter_done, chapter_total, chapter_time_left) or nil
            ...
            if highlight_text ~= "" then
                ...
                if highlight_page then
                    ...
                    if type(highlight_page) == "number" then
                        page_label = string.format(_("第%s页"), tostring(highlight_page))
                    else
